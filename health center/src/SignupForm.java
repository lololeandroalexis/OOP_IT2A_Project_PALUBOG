...
public class SignupForm extends JFrame {
    ...
    // Warning labels for real-time validation
    ...
    public SignupForm() {
        ...
        mobileEmailField.getDocument().addDocumentListener(new DocumentListener() {
            @Override
        ...
        emailWarningLabel.setFont(new Font("Segoe UI", Font.PLAIN, 11));
        emailWarningLabel.setForeground(new Color(220, 53, 69)); // Red color
        emailWarningLabel.setAlignmentX(Component.LEFT_ALIGNMENT);
        contentPanel.add(emailWarningLabel);
        contentPanel.add(Box.createVerticalStrut(10));
        ...
        passwordWarningLabel.setFont(new Font("Segoe UI", Font.PLAIN, 11));
        passwordWarningLabel.setForeground(new Color(220, 53, 69)); // Red color
        passwordWarningLabel.setAlignmentX(Component.LEFT_ALIGNMENT);
        contentPanel.add(passwordWarningLabel);
        contentPanel.add(Box.createVerticalStrut(10));
    ...
    private boolean validateForm() {
        ...
        // Validation 3a: Email validation - must contain "@" symbol if it contains letters
        // If input contains letters, it must be an email and must have "@"
        boolean containsLetters = mobileEmail.matches(".*[a-zA-Z].*");
        if (containsLetters) {
            // This looks like an email, so it MUST contain "@"
            if (!mobileEmail.contains("@")) {
                JOptionPane.showMessageDialog(this, 
                    "Invalid email format. Email must contain '@' symbol (e.g., example@domain.com).", 
                    "Validation Error", 
                    JOptionPane.WARNING_MESSAGE);
                return false;
            }
            // Validate full email format
            if (!isValidEmail(mobileEmail)) {
                JOptionPane.showMessageDialog(this, 
        ...
        // Validation 4a: Password strength validation
    ...
    private boolean isValidPassword(String password) {
        // Regex pattern: must contain uppercase, lowercase, and special characters
        String passwordRegex = "^(?=.*[A-Z])(?=.*[a-z])(?=.*[!@#$%^&*]).*$";
        return password.matches(passwordRegex);
    }

    /**
     * Updates email warning label based on real-time input validation
     */
    private void updateEmailWarning() {
        ...
        // If input contains letters, it must be treated as an email and must contain "@"
        boolean containsLetters = email.matches(".*[a-zA-Z].*");
        
        if (containsLetters) {
            // This looks like an email
            if (!email.contains("@")) {
                emailWarningLabel.setText("⚠ Email must contain '@' symbol (e.g., example@domain.com)");
                return;
            }
            
            // Validate email format
            if (!isValidEmail(email)) {
                emailWarningLabel.setText("⚠ Invalid email format. Use format: example@domain.com");
            } else {
                emailWarningLabel.setText("");
            }
        } else {
            // Pure digits or other format - treat as mobile number, no warning needed
            emailWarningLabel.setText("");
        }
    }

    /**
     * Updates password warning label based on real-time input validation
     */
    private void updatePasswordWarning() {
        String password = new String(passwordField.getPassword()).trim();
        
        if (password.isEmpty()) {
            passwordWarningLabel.setText("");
            passwordWarningLabel.setForeground(new Color(220, 53, 69)); // Reset to red
            return;
        }
        ...
        if (warnings.length() > 0) {
            passwordWarningLabel.setText("⚠ " + warnings.toString());
            passwordWarningLabel.setForeground(new Color(220, 53, 69)); // Red color for warnings
        } else {
            passwordWarningLabel.setText("✓ Password meets all requirements");
            passwordWarningLabel.setForeground(new Color(40, 167, 69)); // Green color for success
        }
    }
